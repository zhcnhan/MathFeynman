...
| POST | `/subjects/{subject_id}/materials/{material_id}/read-pages` | **R57 任务 A · 按需取页范围**：body `{pages?("7-9"), want?}` —— 从缓存里重渲染那几页（PDF 材料）或取原始图片（图片材料）**再读一遍**，按页合并（同页替换、新页追加，幂等）；变更进账本（`pages_reread`，中文） |
