...
| POST | `/subjects/{subject_id}/materials/upload-pages` | **R56 第 3 步 · 图示教材模式（全 AI 模式）导入**：multipart `title?` + `want?` + `files[]`（**页面图片** PNG/JPEG/WebP/GIF，一次 ≤60 页）→ 逐页交给模型读（调用点 `read_page`，走既有审计）→ 用**既有材料层**入库（`kind=pages`、`mode=all_ai`、`page_count`）；**没配能读图的模型 → 中文 422 拒绝且不落库**；读不出来的页照样入库但如实标注（正文 + 账本 `pages_unreadable`）；页面记录另存 `*.pages.json`；返回 `{id,title,page_count,unreadable,pages,elapsed_ms,note_zh,boundary}`（`boundary` ＝ 诚实边界四条，界面直接展示） |
...
| GET | `/subjects/{subject_id}/mode` | **R56 第 3 步**：当前学科走哪条路 + 能不能开图示教材模式：`mode`（`""` / `all_ai`）、`mode_label_zh`、`vision_ready`、`vision_note_zh`（中文原因，含"该去设置里把模型名改成什么"）、`vision_model`、`entry_zh`（导入处要展示的**诚实边界**：做什么 / 三条代价 / 长处 / "不比文字教材更可靠" / "要的是图片、PDF 本身不收"） |
