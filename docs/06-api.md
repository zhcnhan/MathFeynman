...
| POST | `/subjects/{subject_id}/materials/{material_id}/page-mapping` | **R61 任务 A · 旧标签人工指定"当作第几页"**：body `{label:"封面", page_no:4}`。把"标签里没有页号"的旧页**人工定成第 N 页**（写回该页记录：`page_label="第 N 页"`、`original_label`、`manual_page_no`、`page_no_source="manual"`，**`readable` 与其它页不动**），此后这一页**可被正常引用**、也不再出现在"跳过"清单里。**不调用模型**（想让它有内容再点重读）；**幂等**（同一标签同一页号再点一次＝200 + "这一页已经是第 4 页了"，不再记账）。错误全中文：页号非 ≥1 整数 → **422**；标签不存在 → **404**；该页本来就有页号 → **422**；页号已被别的页占用 → **409**（不静默覆盖，提示先撤销那一页）。账本沿用唯一入口 `content_ledger`：`detail.kind="page_mapping"`。返回 `{id,title,label,original_label,page_no,pages,skipped,note_zh}` |
| DELETE | `/subjects/{subject_id}/materials/{material_id}/page-mapping/{label}` | **R61 任务 A · 撤销人工指定**（`label` 需 URL 编码）：还原 `page_label=original_label` 并去掉人工字段 ⇒ 这一页**回到"跳过并列出"**状态；撤销**也留痕**（账本 `detail.kind="page_mapping_undo"`）。非人工指定（或没这个标签）→ **422** 中文"这一页不是人工指定的页号，不能撤销"。返回形状同上（`note_zh` 说明已撤销） |
