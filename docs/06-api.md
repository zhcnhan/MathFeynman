...
| POST | `/subjects/{subject_id}/materials/{material_id}/reparse` | **R55 C4**：给已有材料重做一次"抽取修正"（给 R55 之前导入的旧材料补做用）；**幂等**——第二次 `changed=false`；**不动原始上传文件、不动已生成的内容文件**；无留档时先把当前正文存成 `*.raw.txt` 再改；响应 `{id,title,changed,text_health}` |
