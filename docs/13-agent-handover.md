...
1. **R29 热修（架构侧已修，含回归用例）**：R27 新增的 flow 键在老会话中缺失 →
   `feynman_submit` 500（必现于现存 `s-f2decfcf.u01:a7689b7ebf`）。已加 `_backfill_feynman_keys`
   并在 `step()`/费曼入口/`_ensure_invariants` 三处自愈；**给 Euler**：做一次
   **flow schema 演进系统性排查**（所有"后加且用 `[]` 取值"的键 → 收敛为单一 `_ensure_flow_shape`
   入口 + 缺键/错类型/整块缺失 三类老结构用例），见 docs/09 R29。

2. **无阻塞工单**。R27 遗留微任务（下批，非紧急，见 docs/09 R28）：
...
3. **用户裁定项（F3）**：通过判定现用"账本累计分（维度历轮 max）≥ 阈值"；若要求
   "末次完整稿自身须过线"，改为对本轮 card 单独合成（一行改动）——待用户拍板。
4. 真人浏览器验收（用户动作）：行星科学 u01 走一遍"首讲→补答→整合终讲"，
   确认分数可见上升、缺口提示/额度徽标正确（真模型回归已在临时 DB 通过：0.0 → 补答 0.95/综合 0.38 → 终验 0.73 pass）。
5. 疑点与口径冲突：记 IMPLEMENTATION_NOTES"待架构裁决"；涉及 docs/02/03/05/06/07/14 的语义
   变更在实现时顺带同步。
