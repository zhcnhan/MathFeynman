...
- **工单与汇报的书写格式（2026-09-10 用户明令）**：**禁止使用 Markdown 表格**——
  用户是把内容**复制粘贴**转达给另一个会话的，表格复制会**错行/串列**、信息丢失。
  一律改用**标题 + 无序列表 + 加粗短标签**（如 `- **A1 语义**：…`）；
  需要"对照"时写成并列的列表项，**不要**用 `|` 竖线表。代码与命令用行内反引号或独立代码块。
...
- **界面文案一律"说人话"（2026-09-11 用户明令，见 docs/09 R52）**：
  - **界面上不得出现任何内部编号**：`R38`/`R42`/`§2`/`docs/14 §8`/`Phase B`/`S1-S9` 等
    （**代码注释与文档里可以留**；判据是"会不会渲染到屏幕上"）；
  - **不得出现 schema 版本、字段名、变量名、文件路径**（`schema v1`/`batch_chars`/`trace_path`/`MF_*`…）；
  - **不用工程词当用户标签**：注入预算→**读多少书**、单次调用预算→**每次读多少**、
    总注入上限→**总共最多读多少**、溯源→**来自书的哪里**、覆盖账→**学习进度**、
    节点→**知识点**、字符→**字 / 万字**；
  - **先结论后原因，一句话能说清就别写三句**；不用公文腔；同一概念全程序用同一个词；
  - **换词可以、改含义不行**：像"调小只是分更多批、一章都不会少"这类**真实承诺**不许说软或说丢。
...
> （绝不"前 N 字"）；显式设的上限只作**单次调用预算**（R38 §3 共存口径：**调小不丢章节**）——
> 实测同一本书：注入量 **6,000 → 103,448 字**（2 批）；
...
- `.env`（仓库根，git 忽略）：LLM_API_KEY 等；`MF_AUTO_EXTEND=1` 控制全自动续关；
  ...
  **R42 C3 起后端启动时也会自动清理一次**；**R46 B 起之后每 N 小时再清一次**）、
  `MF_AI_TRACE_CLEAN_INTERVAL_HOURS`（**R46 B 新增**，默认 6；定时清理间隔，非法/≤0 回默认——
  **不许用它把清理静默关掉**，真要不清理请调大保留期）。
