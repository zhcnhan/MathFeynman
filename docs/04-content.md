...
>
> **R37 教材真源化：可答性闸门的**第三类校验**（教材锚定 · Source-First）**：
> 有教材的学科，`taught_facts[].text` 与每道题/每条追问的 `basis.quote` **还必须逐字出自该学科材料正文**
> （`app/content/answerability.py::gate_node(material=…)`，尺子仍是 `content/citations.py` 单一实现）：
> - 事实句在教材里找不到 → **丢弃该事实**；一条不剩 → 调用方判**整单元失败**
>   （`status="uncovered"`，中文告知"教材未覆盖此单元"，**不落盘、不编造**）；
> - 题目/追问引文找不到 → **丢弃该题/该追问**（重试一次后仍不行才丢，不放行）；
> - 违规文案全中文并**说明缺什么**（"教材里没有这句话" vs "本单元对应的教材段落没讲到它"）；
> - **教材原文 > AI 讲解**：讲解只是教材该段的演绎，闸门把"已教集合"从讲解**上移一层**到教材原文
>   （与 R35 的可答性叠成两层：先过"讲解自洽"，再过"教材锚定"）。
> 无教材的学科**不启用**该类校验（退化为 R35 口径），并**显式标注"本内容无教材依据"**（覆盖账本）。
