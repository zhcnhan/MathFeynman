...
> 适用范围：范围：通用核心（状态机/费曼评估/防幻觉/调用点），跨学科复用；rubric 按学科模板化待办
...
[输出纪律] 只输出 JSON，字段见 schema；涉及公式时用 LaTeX；不得输出学生尚未学的结论。
```

- 白名单由程序从图谱取 `node.prereqs ∪ node.core_concepts`，**LLM 无权扩白**。
- 学生问超纲问题（常见于好奇提问）：按禁令礼貌截断，记入画像 `styling_notes`（可成为后续学习的钩子）。

> **R35 可答性（2026-09-10 · 产品级前置条件）**：白名单防的是"讲超纲"，**防不住"问超纲"**。
> 因此凡**向学习者提出的问题**（练习 / socratic / 🤔 小思考 / 追问）都必须能从"已讲内容"推出：
> ① 复述已述事实，或② 由 ≥2 条已述事实经明确规则推出（`taught_facts` + `derivable`）。
> - **出题/小思考生成**（调用点 11 与 1）：必须同时产出 `taught_facts`（逐字取自讲解）与每条问题的
...
> **R37 教材真源化（2026-09-10 · 产品级地基变更）**：R35 保证"讲解与题目自洽"，**不保证"讲解与教材一致"**。
> 因此有教材的学科，真源上移一层：**教材原文 > 用户补充材料 > 大纲 brief > 模型自有知识**
> （自有知识只可用于解释/类比/举例/衔接，**不得引入教材未陈述的事实/数字/结论**）。
> - **起草（调用点 10 `outline_draft`）**：注入**整本书的章 → 节地图** + 本批章节的**完整正文**
>   （`outline/bookmap.py` 解析目录/页首章标题；书太大按章/页边界分批，每批都带全书地图）；
>   单元 `materials[].section` **必须逐字复制**地图条目标签（服务端据此做**全覆盖校验**）；
> - **单元出稿（调用点 11 `unit_content_draft`）**：注入该单元对应章/节的**完整正文**，system prompt 加
>   **教材锚定硬要求**（讲解＝该段教材的完整演绎；`taught_facts[].text` 与 `basis.quote`
>   **逐字摘录自教材原文**；教材没讲到的一律不写不问）；
> - **服务端第三类校验（教材锚定，S5）**：`answerability.gate_node(material=<教材正文>)` ——
>   事实句/引文还必须逐字出自教材；不合规 → 重试一次 → 题目**丢弃**、事实句**整单元失败**
>   （`status="uncovered"`，中文告知"教材未覆盖此单元"，**不落盘、不编造**）；
> - **无教材**时该层校验不启用（退回 R35 口径），并在**覆盖账本**里显式标注"本内容无教材依据"；
> - 引文尺子仍是 `content/citations.py`（单一实现）；R37 给它加了教材排版的归一化
>   （全角 ASCII 折算 + 私用区装饰字形剔除），**不新增第二份实现**。
...
4. service 合成综合分 = Σ(weight×score)；对比 pass_threshold：
   ...
   - **边缘带复评（R30 F6）**：本轮综合分落 `[threshold−0.05, threshold+0.08]` 且本轮非 think
     → 以 think 档对**同一份稿**复评一次，取较高分作为本轮结果（每次提交最多 1 次额外 heavy
     调用；复评失败保留首次结果）。理由：同一篇讲解真模型两次评分波动可达 0.13，落边缘带时
     "这次过、下次不过"（R28 F6）；两次分与采用结论记 `attempts.meta.recheck` + 事件
     `feynman_edge_recheck`。
5. 补答（feynman_answer，学生回答追问）→ 缺口补答评分（轻量评估，非整体重评）：
   只更新该缺口所属维度 → 账本取历轮最高分 → 实时综合分上升可见；答不对缺口保留——
   须**再交一次完整讲解**换取针对该缺口的新追问（F4 文案：不说"稍后可再追一次"）
...
评分防作弊要点：
- rubric 逐维输出 + 原话引文，让评分可审计；evidence 必须出自本轮文本且归一化后 ≥6 字
  （服务端校验：极短引文视为无效并降级，R30 F5）；
