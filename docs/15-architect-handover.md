...
3. **派 R55**（工单已就绪：`.runtime/EULER_TICKET_R55.md`）—— **图示教材引擎**（第三条材料路径）：
   用户拍板"传文件"但**定位为独立引擎、只给非文本/高度依赖图示的教材用**。
   **主路径一个字不改**（覆盖账/逐字可查/本地模型三个保证全在）；
   新引擎必须**显式选择**、必须配视觉模型、**诚实边界写在导入那一刻**
   （能读图和公式，但**没法逐字核对引用**，依据只到**页/图号**）、**不许伪造逐字引文**、
   成本显性（按章只发相关页范围 + 每次调用进审计）。完成后出 **R56 裁决**。
