...
def _draft_messages(spec: dict) -> list[dict]:
    ...
    system = (
        ...
        "      constraint: \"<把题干说的每一个条件都写成约束，如 b % a == 0；没有条件就省略>\"\n"
        "      answer_expr: \"<参数符号表达式，如 a+b>\"\n"
        "      semantics:\n"
        "        expect: \"<标准答案的**独立算式**（可由符号函数表达，如 lcm(a, b)、gcd(a, b)、a*b/100）>\"\n"
        "        requires: [\"<题干里陈述过的每一个条件，如 b % a == 0>\"]\n"
        ...
        "不要用模板+取整实现。\n"
        "**语义自检纪律（R35 §12/§13，违反即拒绝入库）**：① `semantics.expect` 是标准答案的**独立算式**"
        "——**绝不能与 answer_expr 写成同一串文本**（那叫自证、等于没验）；**一步运算也要换个等价写法**："
        "`a+b` → `b + a`、`a-b` → `-(b - a)`、`a*b` → `b * a`、`a/b` → `Rational(a, b)`、"
        "求 LCM 用 `lcm(a,b)`、求 GCD 用 `gcd(a,b)`、百分数用 `a*b/100`；"
        "② 题干里**说过的每一个条件**都必须写进 `requires` **并且**用 `constraint` 强制"
        "（题干不得说出未被约束保证的话：说\"b 是 a 的倍数\"就必须有 b % a == 0）；"
        "③ `semantics.domain` 声明领域谓词：金额/数量/个数题必须 nonneg，个/只/元/颗等离散量必须 integer"
        "（宁可加约束保证整数，也不要出\"半个苹果\"）；④ 题干**只问一个量**（不要同时要「商和余数」两个值）。\n"
