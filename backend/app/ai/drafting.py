...
def _draft_messages(spec: dict) -> list[dict]:
    ...
    system = (
        ...
        "不要用模板+取整实现。\n"
        "**语义自检纪律（R35 §12，违反即拒绝入库）**：① `semantics.expect` 是标准答案的**独立算式**"
        "（不是 answer_expr 的复制——两者不一致会被判缺陷），求 LCM 用 lcm(a,b)、求 GCD 用 gcd(a,b)、"
        "百分数用 a*b/100；② 题干里**说过的每一个条件**都必须写进 `requires` **并且**用 `constraint` 强制"
