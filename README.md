...
10. **学科通用性 = 硬约束，不得为任何单个学科开特例**（2026-09-10 立 · docs/09 R35）：**任何学科**——
    已有的、以后引入的、preset 与自定义、`ai|import|web|mixed` 任何内容源——都走**同一条闭环**与
    **同一套约束**：概念/讲解 → **用自己的话讲出来** → 被追问 → 拓展；且**出题只能问"已经讲过的"**
    （可答性：复述已述事实，或由 ≥2 条已述事实经明确规则推出，否则不许出；**零基础假设**——
    没教过的一律当不会，不假设常识、不假设课外知识）。
    设计的固定追问句："**换个学科还成立吗？**" 不成立即说明存在数学/单科耦合，必须抽离（docs/14）。
...
> 立心批的验收裁决与"工作目录改名 MathFeynman → YanHui"执行记录见 `docs/09-architect-rulings.md` **R32**
> （2026-09-10）；该批只动文档与代码内文案/注释，**未改任何代码逻辑**（基线 pytest 325 passed + 2 skipped 不变）。
